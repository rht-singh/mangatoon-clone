    ...
    fetch(`${baseURL}/api/manga/get_comic?comic_id=${getParams("comic_id")}`)
        ...
        .then((obj) => {
            if (obj.success) {
                ...
                comicImage.src = comic.title_img.replace(
                    "http://165.22.223.28",
                    baseURL
                );

                document.getElementById("comic_name").innerText =
                    comic.comic_title;
                document.getElementById("comic_publisher").innerText =
                    comic.publisher;
                document.getElementById("comic_episodes").innerText =
                    comic.total_episode;
                document.getElementById("comic_language").innerText =
                    comic.language;
